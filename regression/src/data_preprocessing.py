"""
Data Preprocessing Module for Regression Problem
...
class DataPreprocessor:
    """
    Class for preprocessing regression data.
    """
    ...
    def explore_data(self):
        ...
        if self.target_column and self.target_column in self.df.columns:
            print(f"\n8. Target Statistics ({self.target_column}):")
            print(f"   Mean: {self.df[self.target_column].mean():.4f}")
            print(f"   Median: {self.df[self.target_column].median():.4f}")
            print(f"   Std: {self.df[self.target_column].std():.4f}")
            print(f"   Min: {self.df[self.target_column].min():.4f}")
            print(f"   Max: {self.df[self.target_column].max():.4f}")
